...
model = ModelUtil.create('Model_SPO_MIT_distribute');
...
model.param.set('SPO_dis_mit2chl_rt', [num2str(SPO_dis_mit2chl_rt)]);
...
for loop=1:Num_spo
    ...
    model.component('comp1').geom('geom1').feature(tmptag_spo).set('r', 'SPO_MS_radius-SPO_dis_chl2wall-SPO_thick_chl_layer-SPO_MS_radius*(SPO_dis_mit2chl_rt)');
...
for loop_i=1:Num_spo
    ...
    for tmp_loop_j=1:numel(tmp_idx_contact)
        ...
        if loop_j > loop_i
            ...
            %%%for SPO CHL
            %%%shift wp toward/backward by distance SPO_dis_chl2wall+SPO_thick_chl_layer+SPO_MS_radius*(SPO_dis_mit2chl_rt)
            tmp_pt1_a=tmp_pt1+tmp_vec1/norm(tmp_vec1)*(SPO_dis_chl2wall+SPO_thick_chl_layer+SPO_MS_radius*(SPO_dis_mit2chl_rt));
            tmp_pt1_b=tmp_pt1-tmp_vec1/norm(tmp_vec1)*(SPO_dis_chl2wall+SPO_thick_chl_layer+SPO_MS_radius*(SPO_dis_mit2chl_rt));
...
for loop_i=1:Num_spo
    if mat_contact_cell_idx(loop_i,loop_i)~=0 % not contact with box
        ...
        SPO_MIT_tag_set{loop_i}=tmptag_int;
        %%% delete csur
        tmptag_del=['del', num2str(count_del)];count_del=count_del+1;
        model.component('comp1').geom('geom1').create(tmptag_del, 'Delete');
        model.component('comp1').geom('geom1').feature(tmptag_del).selection('input').init;
        ...
        SPO_MIT_tag_set{loop_i}=tmptag_csur;
    end
end
tmptag_del=['del', num2str(count_del)];count_del=count_del+1;
model.component('comp1').geom('geom1').create(tmptag_del, 'Delete');
...
count_mesh_size=1;
count_mesh_ftri=1;
SPO_MIT_center_trimesh_pts={};%store chloroplasts centroid for all SPO cells
SPO_MIT_center_trimesh_tri={};
for loop_i=1:numel(TagsCreSel)
    ...
    model.component('comp1').mesh(tmptag_mesh).feature(tmptag_size).set('hmin', SPO_diameter_mit_patch/10);
    model.component('comp1').mesh(tmptag_mesh).feature(tmptag_size).set('hmaxactive', true);
    model.component('comp1').mesh(tmptag_mesh).feature(tmptag_size).set('hmax', SPO_diameter_mit_patch/2);%max distance allow
    ...
    SPO_MIT_center_trimesh_pts{loop_i}=pts;
    SPO_MIT_center_trimesh_tri{loop_i}=tri;
end
...
SPO_MIT_center_final_pts={};
for loop_i=1:numel(TagsCreSel)
    tmp_pts=SPO_MIT_center_trimesh_pts{loop_i};
    tmp_tri=SPO_MIT_center_trimesh_tri{loop_i};
    ...
    while all(tmp_flag_pts==2,'all')==0
        ...
        if isempty(tmp_x3)==1
            ...
            for loop_pts=1:numel(tmp_x3)
                ...
                if any(tmp_dis_sq_all<=SPO_diameter_mit_patch^2)
    ...
    SPO_MIT_center_final_pts{loop_i}=tmp_sel_pts;
end
...
save tmp_SPO_MIT_distribute.mat -regexp '^(?!(model|ans)$).'
disp('Potential centroids for mitochondria in Spongy are ready.')
mphsave(model, 'tmp_SPO_MIT_geo.mph')

			...
			for(k_pts=0;k_pts<curchlo->S->pnum;k_pts++)
			{
				...
				//// for loop each triangle of current MS, check how many intersection
					...
					if(tmp2_count_isectpts==1)
					{
						if(i<=pal_num)
						{
							//printf("PAL_MS_%d_CHL_%d.ply PTS %d is inside PAL_MS_%d.ply\n",i,j,k_pts,i);
						}
						else
						{
							//printf("SPO_MS_%d_CHL_%d.ply PTS %d is inside SPO_MS_%d.ply\n",i-pal_num,j,k_pts,i-pal_num);
						}
					}
					if(tmp2_count_isectpts==0||tmp2_count_isectpts==2)
					{
						if(i<=pal_num)
						{
							printf("[Warning] PAL_MS_%d_CHL_%d.ply PTS %d is not inside PAL_MS_%d.ply\n",i,j,k_pts,i);
						}
						else
						{
							printf("[Warning] SPO_MS_%d_CHL_%d.ply PTS %d is not inside SPO_MS_%d.ply\n",i-pal_num,j,k_pts,i-pal_num);
						}
					}
					if(tmp2_count_isectpts>=3)
					{
						if(i<=pal_num)
						{
							printf("[Warning][count_isectpts=%d] PAL_MS_%d_CHL_%d.ply PTS %d is not fully inside PAL_MS_%d.ply\n",tmp_count_isectpts,i,j,k_pts,i);
						}
						else
						{
							printf("[Warning][count_isectpts=%d] SPO_MS_%d_CHL_%d.ply PTS %d is not fully inside SPO_MS_%d.ply\n",tmp_count_isectpts,i-pal_num,j,k_pts,i-pal_num);
						}
					}
				}
				free(curray);

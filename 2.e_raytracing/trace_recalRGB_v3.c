...
* 2022-July
* [v3]
* to further accelerate the calculation on cluster
* replace fscanf() with fgets()+strtok()+strtod()
***********************************************/

/***********************************************
* to accelerate the calculation for RT training
* This is a version which takes all SACs for Red/Green/Blue light
* and recalculate the ray tracing
* Equal to, run trace_recal for three times
* Save time for reading light path repeatedly
***********************************************/

/***********************************************
*    version 2022 June
*    - accept count_chl4RT file format which assign different [chl] to each
*      chloroplast in the same palisade/spongy mesophyll cell
*    - ccompatible with old count_chl4RT format that forces to assign same
*      [chl] to all chloroplasts in one mesophyll cell
*
*    version 2021 July
...
int main(int argc, char **argv)
{
	...
	int tmp_pal_num,tmp_spo_num;

	char tmp_buf[128];
	char *tmp_token;
	char tmp2_token[11][20];//store token from strtok()
	int idx_token;
	char *tmp_token_ep;
	...
	****************************************************/
	...
	//////////////////////read in count_chl///////////////
	fp=fopen(*++argv,"r");
	fscanf(fp,"%d %d\n",&tmp_pal_num,&tmp_spo_num);
    // test fscanf, check the format of file count_chl
	fscanf(fp,"%d %lf",&(ms_chl_num[0]),&(ms_chl_con[0]));
	// if getchar()=='\n', then old count_chl format
	// else if getchar()='\0', then new chl format
	tmp_char=fgetc(fp);
	if(tmp_char=='\n')
	{
		flag_format_count_chl=1;
		// store [chl] information to ms_chl_con[ms_num]
		for(loop1=2;loop1<=(ms_num);loop1++)
		{
	...
	{
		// store [chl] information to ms_chl_con_v2[ms_num][ms_max_chl_num]
		flag_format_count_chl=2;
		ms_chl_con_v2[0][0]=ms_chl_con[0];
		//// read the remaining of this 1st MSC
		for(loop2=2;loop2<=ms_chl_num[0];loop2++)
		{
			fscanf(fp,"%lf%c",&(ms_chl_con_v2[0][loop2-1]),&tmp_char);
		}
		fgetc(fp);
		//// read the rest MSC
		for(loop1=2;loop1<=(ms_num);loop1++)
		{
			fscanf(fp,"%d%c",&(ms_chl_num[loop1-1]),&tmp_char);
			for(loop2=1;loop2<=ms_chl_num[loop1-1];loop2++)
			{
				fscanf(fp,"%lf%c",&(ms_chl_con_v2[loop1-1][loop2-1]),&tmp_char);
			}
			fgetc(fp);
		}
	}
	...
	{
		flag_format_count_chl=0;
		if(flag_errormsg==1)
		{
			printf("Fail to read file count_chl4RT\n");
		}
	}
	...
	//////////////////////////read in MS///////////////////////	
	if(flag_RTtrain_mod==0)
	{
		printf("read in MS\n");
	}
	count_ms=0;	
	//PAL
	for(loop1=1;loop1<=(pal_num);loop1++)
	{	   
		count_ms++;
		/********************
		**cell
		********************/
		strcpy(cell,"MS/PAL_MS_");
	...
	//SPO
	for(loop1=1;loop1<=(spo_num);loop1++)
	{	   
		count_ms++;
		/********************
		**cell
		********************/
		strcpy(cell,"MS/SPO_MS_");
		sprintf(str_num,"%d",loop1);
		strcat(cell,str_num);
		strcat(cell,".ply");
	...
	count_nonms=0;	
	//EPL_l
	for(loop1=1;loop1<=epl_l_num;loop1++)//9 upper epidermis
	{
		count_nonms++;
		//////////read in cell/////////
		strcpy(cell,"nonMS/EPL_l_");
		sprintf(str_num,"%d",loop1);
		strcat(cell,str_num);		
		strcat(cell,".ply");
		//printf("cell:%s\n",cell);
	...
	//EPL_u
	for(loop1=1;loop1<=epl_u_num;loop1++)//9 upper epidermis
	{
		count_nonms++;
		//////////read in cell/////////
		strcpy(cell,"nonMS/EPL_u_");
		sprintf(str_num,"%d",loop1);
		strcat(cell,str_num);		
		strcat(cell,".ply");
		//printf("cell:%s\n",cell);
	...
	for(loop1=1;loop1<=ms_num;loop1++)
	{
		...
		for(i=1;i<=ms_chl_num[loop1-1];i++)
		{
			...
			if(flag_format_count_chl==1)
			{
				curobj->sac_i=SAC_chlab*ms_chl_con[loop1-1]; // same concentration for all chloroplasts
			}
			else if(flag_format_count_chl==2)
			{
				curobj->sac_i=SAC_chlab*ms_chl_con_v2[loop1-1][i-1];
			}
			else
			{
				if(flag_errormsg==1)
	...
	*****************************************************************/
	...
	for(loop_file=1;loop_file<=max_file_idx;loop_file++)
	{
		...
		else
		{
			...
			{
				tmp_token=strtok(tmp_buf," ");
				idx_token=0;
				strcpy(tmp2_token[idx_token],tmp_token);
				idx_token=idx_token+1;
				while (tmp_token != NULL && idx_token<=10)
				{
					tmp_token=strtok(NULL," ");
					strcpy(tmp2_token[idx_token],tmp_token);
					idx_token=idx_token+1;
    				}
				ray_i=atoi(tmp2_token[0]);
				ray_j=atoi(tmp2_token[1]);
				dis=strtod(tmp2_token[2],&tmp_token_ep);
				P_celltype=atoi(tmp2_token[3]);
				P_cellname=atoi(tmp2_token[4]);
				P_chlname=atoi(tmp2_token[5]);
				P_vacuolename=atoi(tmp2_token[6]);
				P_tri=atoi(tmp2_token[7]);
				P_outin_P=atoi(tmp2_token[8]);
				tmp_I=strtod(tmp2_token[9],&tmp_token_ep);
				ray_status=atoi(tmp2_token[10]);
	...
	*****************************************************************/
	...
	/****************************************************
	* Green light = 560nm
	* - reinitialize SAC for chloroplasts SAC
	* - recalculate RT
	* - skip reading ply files
	* - output results
	****************************************************/
	start=time(NULL);
	SAC_water=atof(*++argv);//Now, input unit m-1, previously, 1e2*atof(*++argv);

...
int main(int argc, char **argv)
{
	...
	//EPL_u
	for(loop1=1;loop1<=epl_u_num;loop1++)//upper epidermis
	{
		count_nonms++;
		//////////read in cell/////////
	...
	ray_j_end=atoi(*++argv);
	double dx=(xmax-xmin)/(cutnum_x+1),dy=(ymax-ymin)/(cutnum_y+1);
/*		
	Ray *curray;
...
*/
	...
	for(ray_i=ray_i_start;ray_i<ray_i_end;ray_i++) ///// 0 < cutnum
	{
		for(ray_j=ray_j_start;ray_j<ray_j_end;ray_j++)
		{
			...
			curray->P[1]=ymin+(ray_j+1)*dy;
			curray->P[2]=zmax-0.05e-6;
		
			curray->D[0]=0.0;
			curray->D[1]=0.0;
			curray->D[2]=-1.0;
			curray->P_tri=10;//previously -1; but cause issue to trace_recal() based on absorbevent files
			...
			if(flag_pertube_dir==1)
			{
				...
				//printf("alpha:%e\n",alpha);
				curray->D[2]=-cos(alpha/180*PI);
				beta=rand()/360;
				curray->D[0]=sin(alpha/180*PI)*sin(beta/180*PI);
				curray->D[1]=sin(alpha/180*PI)*cos(beta/180*PI);
			}
